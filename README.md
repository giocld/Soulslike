# RUP Soulslike Game(Low-poly soulslike game)
**Built with C# and Unity**  
---
**!! kyriws gia ta docs kai gia ena backup einai, xrhsimopoihste to internal vc tou unity !!**
---
**START:** A village where the people are cursed
### Main Quest
Kill the boss in the castle to free the people from the curse
### Side Quest
Kill the other boss to find the husband
---
## NPCs
| NPC | Role |
|-----|------|
| **Blacksmith** | Weaponry |
| **Woman** | Potions |
| **Old Man** | Armory |
| **Old Woman** | Info extraction |
---
## Weapons
### Starting Choice
1. **Dagger**
   - Bleed (coating)
   - Poison (glow)
2. **Sword**
   - Bleed (coating)
   - Flame (fire)
### Lootable
- **Two-hand Axe**
  - Bleed (coating)
  - 2 blades, dmg
---
## Items
### Potions
- Health potion
---
## Enemies
| Enemy | Description | Weapons |
|-------|-------------|---------|
| **Goblins** | Slim, fast | Dagger, Bow |
| **Skeletons** | Bones | Sword & shield, Spear |
---
## Bosses
| Boss | Description | Weapon |
|------|-------------|--------|
| **Goblin Boss / Orc** | Big, fat, muscle, slow | Fists |
| **Skeleton King** | Final boss | Greatsword |
